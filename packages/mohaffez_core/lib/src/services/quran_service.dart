...
class QuranService {
  ...
  /// Resolves the Mushaf page for a structured assignment created by the
  /// teacher. The lookup uses the bundled Quran map, so it performs no network
  /// or Firestore request. If the ayah cannot be resolved, the Surah start page
  /// is returned as a safe fallback.
  Future<int?> findPageForAssignment(
    String surahName, {
    String? fromAyah,
  }) async {
    final surahNumber = surahNumberForName(surahName);
    if (surahNumber == null) return null;

    final ayahNumber = _parseLocalizedInt(fromAyah);
    if (ayahNumber == null) {
      return surahStartPages[surahNumber];
    }

    if (!_isLoaded) await _loadQuranData();
    ...
    if (pages != null) {
      for (final rawPage in pages) {
        ...
        final verses = page['verses'] as List? ?? const [];
        final containsAyah = verses.whereType<Map>().any((rawVerse) {
          final verse = Map<String, dynamic>.from(rawVerse);
          final verseSurah = (verse['surah'] as num?)?.toInt();
          final verseAyah = (verse['ayahNumber'] as num?)?.toInt() ??
              (verse['verse'] as num?)?.toInt();
          return verseSurah == surahNumber && verseAyah == ayahNumber;
        });
        if (containsAyah) {
          final resolvedPage = (page['page'] as num?)?.toInt();
          if (resolvedPage != null && isValidPage(resolvedPage)) {
            return resolvedPage;
          }
        }
      }
  ...
  int? surahNumberForName(String value) {
    final target = _normalizeSurahName(value);
    if (target.isEmpty) return null;
    for (final entry in surahNames.entries) {
      if (_normalizeSurahName(entry.value) == target) return entry.key;
    }
    return null;
  }
  ...
  List<Map<String, dynamic>> _getVersesForPage(int pageNumber) {
    ...
    try {
      ...
      return verses.map((v) => Map<String, dynamic>.from(v as Map)).toList();
    } catch (e) {

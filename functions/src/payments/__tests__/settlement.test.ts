...
describe("Next scheduled settlement date", () => {
  ...
  it("June 24 points to July 1, not 14 days later", () => {
    expect(
      cairoParts(nextScheduledSettlementAt(new Date("2026-06-24T09:00:00Z"))),
    ).toEqual({
      year: 2026,
      month: 7,
      day: 1,
  ...
  it("Before the 15th settlement time points to the 15th", () => {
    expect(
      cairoParts(nextScheduledSettlementAt(new Date("2026-06-14T21:00:00Z"))),
    ).toEqual({
      year: 2026,
      month: 6,
      day: 15,
  ...
  it("After the 15th settlement time points to the 1st of next month", () => {
    expect(
      cairoParts(nextScheduledSettlementAt(new Date("2026-06-14T22:00:00Z"))),
    ).toEqual({
      year: 2026,
      month: 7,
      day: 1,
...
describe("Settlement payout — worked examples from the checklist", () => {
  ...
  it("Zero pending → zero everything (no-op settlement is safe)", () => {
    const { commissionPiastres, teacherNetPiastres } =
      computeSettlementPiastres(0, 0.1);
...
describe("Documentation consistency — default tier values", () => {
  ...
  it("rates are 15 / 12 / 10 / 8 / 5 percent", () => {
    expect(DEFAULT_TIERS.map((t) => t.rate)).toEqual([
      0.15, 0.12, 0.1, 0.08, 0.05,
    ]);
  });
